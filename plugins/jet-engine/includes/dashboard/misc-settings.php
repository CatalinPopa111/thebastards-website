...
if ( ! class_exists( 'Jet_Engine_Misc_Settings' ) ) {
	...
	class Jet_Engine_Misc_Settings {
			...
			>
				<div class="jet-engine-misc">
					...
					<cx-vui-select
						...
						description="<?php _e( 'Choose a library that is used for the slider functionality in Listing Grid and Dynamic Field widgets. For now, Slick.js will be used on all installs as default, but in the future all new installs will be switched to Swiper.js (with the ability to switch back to Slick). If you do not plan to migrate to Swiper on this website, please, select \'Slick.js\' option explicitly.', 'jet-engine' ); ?>"
						...
						:options-list="[
							{
								value: '',
								label: '<?php esc_html_e( 'Default (Slick.js)', 'jet-engine' ); ?>',
							},
							{
								value: 'swiper',
		...
		public function get_settings( $setting = null, $settings = false ) {
			...
			if ( empty( $settings['slider_library'] ) ) {
				$settings['slider_library'] = 'slick';
			}
