	...
	<div class="jet-container">
		<div class="jet-header">
			<div class="jet-header__heading">
				...
				<h3><?php _e( '<span>JetBooking</span> / Setup Wizard', 'jet-booking' ); // phpcs:ignore ?></h3>
			</div>
		...
		<div class="jet-content">
			<template v-if="! isSet || isReset">
				<template v-if="! currentStep">
					<div class="jet-content__title">
						<h1><?php esc_html_e( 'Welcome to JetBooking Setup Wizard', 'jet-booking' ); ?></h1>
					</div>

					<div class="jet-content__heading">
						<h2><?php esc_html_e( 'Choose Booking Mode', 'jet-booking' ); ?></h2>
					</div>
					...
					<div class="jet-content__body">
						...
						>
							<cx-vui-radio
								:options-list="[ {
									value: 'plain',
									label: '<?php esc_html_e( 'Plain mode', 'jet-booking' ); ?>',
								} ]"
								v-model="setupData.booking_mode"
							/>

							<div class="cx-vui-component__desc">
								<?php esc_html_e( 'A flexible booking setup. It uses separate CPTs for bookings and orders (both JetEngine-built and not), a customizable booking form, and doesn\'t require additional plugins.', 'jet-booking' ); ?>
							</div>
						</div>
						...
						<div>
							...
							>
								<cx-vui-radio
									:options-list="[ {
											value: 'wc_based',
											label: '<?php esc_html_e( 'WooCommerce-based mode', 'jet-booking' ); ?>',
										} ]"
									v-model="setupData.booking_mode"
								/>

								<div class="cx-vui-component__desc">
									<?php esc_html_e( 'Creates a deep WooCommerce integration and configures everything from Products CPT to checkout and payments. Easy setup compared to Plain mode.', 'jet-booking' ); ?>
								</div>
							</div>

							<div v-if="! hasWoocommerce" class="jet-setup-notice jet-setup-notice-warning">
								<?php printf(
									__( 'WooCommerce plugin isn\'t installed. To activate the mode, you need to <a href="%s" target="_blank">install the plugin</a> and refresh the page.', 'jet-booking' ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					...
					<div class="jet-content__actions">
						<cx-vui-button button-style="accent" :loading="loading" @click="nextStep">
							<template slot="label">
								<?php esc_html_e( 'Start setup', 'jet-booking' ); ?>
							</template>
						</cx-vui-button>
					</div>
				</template>
				...
				<template v-if="1 === currentStep">
					<div class="jet-content__heading">
						<h2><?php esc_html_e( 'Set Up Booking Instance ', 'jet-booking' ); ?></h2>
						<p class="jet-content__heading-desc">
							<?php esc_html_e( 'Select a Custom Post Type (CPT) for the booking instance. For instance, the "Apartments" Post Type is for apartment bookings and the "Cars" CPT is for car bookings.', 'jet-booking' ); ?>
						</p>
					</div>
						...
						<div class="cx-vui-component cx-vui-component--equalwidth">
							<div class="cx-vui-component__meta">
								<label class="cx-vui-component__label">
									<?php esc_html_e( 'Booking Post Type', 'jet-booking' ); ?>
								</label>
							</div>
							...
							<div class="cx-vui-component__control jet-advanced-select">
								<?php if ( class_exists( 'Jet_Engine' ) ) : ?>
									<cx-vui-button
										...
										url="<?php  echo esc_url( add_query_arg( [ 'page' => 'jet-engine-cpt', 'cpt_action' => 'add' ], admin_url( 'admin.php' ) ) ); ?>"
										target="_blank"
									>
										<template slot="label">
											<?php esc_html_e( 'Create new post type', 'jet-booking' ); ?>
								...
								<cx-vui-f-select
									...
									placeholder="<?php esc_html_e( 'Select post type', 'jet-booking' ); ?>"
				...
				<template v-if="2 === currentStep">
					<div class="jet-content__heading">
						<h2><?php esc_html_e( 'Set Up Orders', 'jet-booking' ); ?></h2>

						<p class="jet-content__heading-desc">
							<?php _e( 'Choose a Custom Post Type (CPT) for storing booking orders. Orders will be available in the WordPress dashboard and the default WooCommerce Orders CPT. <i>If you enable WooCommerce basic integration, it will be impossible to select the CPT</i>.', 'jet-booking' ); // phpcs:ignore ?>
						</p>
					</div>
					...
					<div class="jet-content__body">
						<div :class="[ 'cx-vui-component', 'cx-vui-component--equalwidth', {'disabled': setupData.wc_integration } ]">
							<div class="cx-vui-component__meta">
								<label class="cx-vui-component__label">
									<?php esc_html_e( 'Order Post Type', 'jet-booking' ); ?>
								</label>

								<p class="cx-vui-component__desc">
									<?php esc_html_e( 'Select a Custom Post Type where booking orders will be stored.', 'jet-booking' ); ?>
								</p>
							</div>
						...
						<cx-vui-switcher
							...
							label="<?php esc_html_e( 'WooCommerce basic integration', 'jet-booking' ); ?>"
							description="<?php esc_html_e( 'Enable the toggle if you want to integrate booking orders with WooCommerce.', 'jet-booking' ); ?>"
						...
						<div v-if="! hasWoocommerce" class="jet-setup-notice jet-setup-notice-warning">
							<?php printf(
								__( 'WooCommerce plugin isn\'t installed. To enable the integration, you need to <a href="%s" target="_blank">install the plugin</a> and refresh the page.', 'jet-booking' ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
								esc_url( add_query_arg( [ 's' => 'woocommerce', 'tab' => 'search', 'type' => 'term' ], admin_url( 'plugin-install.php' ) ) )
							); ?>
						</div>
						...
						<cx-vui-switcher
							...
							description="<?php esc_html_e( 'Enable this toggle to update WooCommerce order status on the booking status change (by default, the booking status change doesn\'t affect the related order).', 'jet-booking' ); ?>"
							:wrapper-css="[ 'equalwidth' ]"
				...
				<template v-if="3 === currentStep">
					<div class="jet-content__heading">
						<h2><?php esc_html_e( 'Set Up Database Tables', 'jet-booking' ); ?></h2>
						<p class="jet-content__heading-desc">
							<?php esc_html_e( 'Generate default and add custom database columns for storing booking details. All columns will appear in the plugin Settings Tools tab, and their data will be stored in the associated database.', 'jet-booking' ); ?>
						</p>
					</div>
					...
					<div class="jet-content__body">
						<div class="cx-vui-component cx-vui-component--equalwidth">
							<div class="cx-vui-component__meta">
								<label class="cx-vui-component__label">
									<?php esc_html_e( 'Default database columns', 'jet-booking' ); ?>
								</label>

								<div class="cx-vui-component__desc">
									<?php esc_html_e( 'These are default database columns created for you automatically.', 'jet-booking' ); ?>
								</div>
							</div>
							<div class="cx-vui-component__control">
						...
						<div class="cx-vui-component cx-vui-component--equalwidth">
							<div class="cx-vui-component__meta">
								<label class="cx-vui-component__label">
									<?php esc_html_e( 'Custom database columns', 'jet-booking' ); ?>
								</label>
								<div class="cx-vui-component__desc">
									<?php esc_html_e( 'Add more database columns if needed. Spaces will be converted to "_" .', 'jet-booking' ); ?>
								</div>
							</div>
							<div class="cx-vui-component__control">
								<ul v-if="additionalDBColumns.length" class="jet-abaf-setup__custom_db-columns">
									...
									>
										<cx-vui-input
											...
											placeholder="<?php esc_html_e( 'Enter column name', 'jet-booking' ) ?>"
										></cx-vui-input>
										...
										<div class="jet-abaf-setup__custom_db-column-actions">
											<div class="jet-abaf-setup__custom_db-column-actions-info">
												...
												<div class="cx-vui-tooltip"><?php esc_html_e( 'Copy column', 'jet-booking' ); ?></div>
											</div>
											...
											<div class="jet-abaf-setup__custom_db-column-actions-info">
												...
												<div class="cx-vui-tooltip"><?php esc_html_e( 'Delete column', 'jet-booking' ); ?></div>
											</div>
										</div>
									</li>
								</ul>
								...
								<cx-vui-button button-style="link-accent" @click="addNewColumn">
									<template slot="label">
										...
										<?php esc_html_e( 'Add new column', 'jet-booking' ); ?>
									</template>
								</cx-vui-button>
							</div>
						</div>
				...
				<template v-if="4 === currentStep">
					<div class="jet-content__heading">
						<h2><?php esc_html_e( 'Booking Form Settings', 'jet-booking' ); ?></h2>

						<p class="jet-content__heading-desc">
							<?php _e( 'Generate a standard booking form and add it to the booking instance single page. The form will appear in the <b>JetFormBuilder > Forms tab</b>.', 'jet-booking' ); // phpcs:ignore ?>
						</p>
					</div>
					...
					<div class="jet-content__body">
						<cx-vui-switcher
							...
							description="<?php esc_html_e( 'Enable to generate a single page booking form.', 'jet-booking' ); ?>"
							:wrapper-css="[ 'equalwidth' ]"
							v-model="setupData.create_single_form"
						></cx-vui-switcher>

						<div v-if="setupData.create_single_form" class="jet-setup-notice">
							<?php esc_html_e( 'For form creation purposes, a free JetFormBuilder plugin will be installed and activated.', 'jet-booking' ); ?>
						</div>
					</div>
				</template>
				...
				<template v-if="4 < currentStep">
					<div class="jet-content__title">
						<h1><?php esc_html_e( 'Finish Setup', 'jet-booking' ); ?></h1>
					</div>

					<div class="jet-content__heading">
						<h2><?php esc_html_e( 'Congrats, you finished the setup', 'jet-booking' ); ?></h2>

						<p class="jet-content__heading-desc">
							<?php _e( 'These are all settings you configured during the setup. You can adjust them in the Booking Settings if needed. Please remember you can disable Setup Wizard in the <b>Booking Settings</b> > <b>Advanced tab</b>.', 'jet-booking' ); // phpcs:ignore ?>
						</p>
					</div>
					...
					<div class="jet-content__body wide">
						<div class="main">
							<template v-if="'plain' === setupData.booking_mode">
								<div class="cx-vui-component cx-vui-component--equalwidth">
									<div class="cx-vui-component__meta">
										<label class="cx-vui-component__label">
											<?php esc_html_e( 'Custom post type settings', 'jet-booking' ); ?>
										</label>

										<div class="cx-vui-component__desc">
											<?php esc_html_e( 'Selected custom post types.', 'jet-booking' ); ?>
										</div>
									</div>

									<div class="cx-vui-component__control actions-list">
										<template v-for="instance in log.instances" :key="instance.name">
											<cx-vui-button
												button-style="link-accent"
												tag-name="a"
												target="_blank"
												:url="instance.edit_link"
											>
												<template slot="label">
													<?php esc_html_e( 'Manage bookings', 'jet-booking' ); ?> {{instance.name}}
													<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
														<path d="M3.2513 9.99967C3.2513 8.57467 4.40964 7.41634 5.83464 7.41634H9.16797V5.83301H5.83464C3.53464 5.83301 1.66797 7.69967 1.66797 9.99967C1.66797 12.2997 3.53464 14.1663 5.83464 14.1663H9.16797V12.583H5.83464C4.40964 12.583 3.2513 11.4247 3.2513 9.99967ZM6.66797 10.833H13.3346V9.16634H6.66797V10.833ZM14.168 5.83301H10.8346V7.41634H14.168C15.593 7.41634 16.7513 8.57467 16.7513 9.99967C16.7513 11.4247 15.593 12.583 14.168 12.583H10.8346V14.1663H14.168C16.468 14.1663 18.3346 12.2997 18.3346 9.99967C18.3346 7.69967 16.468 5.83301 14.168 5.83301Z" fill="#0D88BF"></path>
													</svg>
												</template>
										...
										>
											<template slot="label">
												<?php esc_html_e( 'Manage orders', 'jet-booking' ); ?>
												<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
													<path d="M3.2513 9.99967C3.2513 8.57467 4.40964 7.41634 5.83464 7.41634H9.16797V5.83301H5.83464C3.53464 5.83301 1.66797 7.69967 1.66797 9.99967C1.66797 12.2997 3.53464 14.1663 5.83464 14.1663H9.16797V12.583H5.83464C4.40964 12.583 3.2513 11.4247 3.2513 9.99967ZM6.66797 10.833H13.3346V9.16634H6.66797V10.833ZM14.168 5.83301H10.8346V7.41634H14.168C15.593 7.41634 16.7513 8.57467 16.7513 9.99967C16.7513 11.4247 15.593 12.583 14.168 12.583H10.8346V14.1663H14.168C16.468 14.1663 18.3346 12.2997 18.3346 9.99967C18.3346 7.69967 16.468 5.83301 14.168 5.83301Z" fill="#0D88BF"></path>
												</svg>
											</template>
								...
								<template v-if="log.wc_integration.enabled">
									...
									<div class="cx-vui-component cx-vui-component--equalwidth">
										<div class="cx-vui-component__meta">
											<label class="cx-vui-component__label">
												<?php esc_html_e( 'WooCommerce integration', 'jet-booking' ); ?>
											</label>

											<div class="cx-vui-component__desc">
												<?php esc_html_e( 'Proceed to manage related product and booking orders.', 'jet-booking' ); ?>
											</div>
										</div>
										...
										<div class="cx-vui-component__control actions-list">
											...
											>
												<template slot="label">
													<?php esc_html_e( 'Related product', 'jet-booking' ); ?>
													<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
														<path d="M3.2513 9.99967C3.2513 8.57467 4.40964 7.41634 5.83464 7.41634H9.16797V5.83301H5.83464C3.53464 5.83301 1.66797 7.69967 1.66797 9.99967C1.66797 12.2997 3.53464 14.1663 5.83464 14.1663H9.16797V12.583H5.83464C4.40964 12.583 3.2513 11.4247 3.2513 9.99967ZM6.66797 10.833H13.3346V9.16634H6.66797V10.833ZM14.168 5.83301H10.8346V7.41634H14.168C15.593 7.41634 16.7513 8.57467 16.7513 9.99967C16.7513 11.4247 15.593 12.583 14.168 12.583H10.8346V14.1663H14.168C16.468 14.1663 18.3346 12.2997 18.3346 9.99967C18.3346 7.69967 16.468 5.83301 14.168 5.83301Z" fill="#0D88BF"></path>
													</svg>
												</template>
								...
								<template v-if="log.form.hasOwnProperty( 'link' )">
									...
									<div class="cx-vui-component cx-vui-component--equalwidth">
										<div class="cx-vui-component__meta">
											<label class="cx-vui-component__label">
												<?php esc_html_e( 'Form settings', 'jet-booking' ); ?>
											</label>

											<div class="cx-vui-component__desc">
												<?php esc_html_e( 'Proceed to customize the form.', 'jet-booking' ); ?>
											</div>
										</div>
										...
										<div class="cx-vui-component__control actions-list">
											...
											>
												<template slot="label">
													<?php esc_html_e( 'Link to created form', 'jet-booking' ); ?>
													<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
														<path d="M3.2513 9.99967C3.2513 8.57467 4.40964 7.41634 5.83464 7.41634H9.16797V5.83301H5.83464C3.53464 5.83301 1.66797 7.69967 1.66797 9.99967C1.66797 12.2997 3.53464 14.1663 5.83464 14.1663H9.16797V12.583H5.83464C4.40964 12.583 3.2513 11.4247 3.2513 9.99967ZM6.66797 10.833H13.3346V9.16634H6.66797V10.833ZM14.168 5.83301H10.8346V7.41634H14.168C15.593 7.41634 16.7513 8.57467 16.7513 9.99967C16.7513 11.4247 15.593 12.583 14.168 12.583H10.8346V14.1663H14.168C16.468 14.1663 18.3346 12.2997 18.3346 9.99967C18.3346 7.69967 16.468 5.83301 14.168 5.83301Z" fill="#0D88BF"></path>
													</svg>
												</template>
							...
							<template v-else>
								<div class="cx-vui-component cx-vui-component--equalwidth">
									<div class="cx-vui-component__meta">
										<label class="cx-vui-component__label">
											<?php esc_html_e( 'WooCommerce mode settings', 'jet-booking' ); ?>
										</label>

										<div class="cx-vui-component__desc">
											<?php esc_html_e( 'Set additional WooCommerce mode settings.', 'jet-booking' ); ?>
										</div>
									</div>
									...
									<div class="cx-vui-component__control actions-list">
										...
										>
											<template slot="label">
												<?php esc_html_e( 'Manage all bookings', 'jet-booking' ); ?>
												<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
													<path d="M3.2513 9.99967C3.2513 8.57467 4.40964 7.41634 5.83464 7.41634H9.16797V5.83301H5.83464C3.53464 5.83301 1.66797 7.69967 1.66797 9.99967C1.66797 12.2997 3.53464 14.1663 5.83464 14.1663H9.16797V12.583H5.83464C4.40964 12.583 3.2513 11.4247 3.2513 9.99967ZM6.66797 10.833H13.3346V9.16634H6.66797V10.833ZM14.168 5.83301H10.8346V7.41634H14.168C15.593 7.41634 16.7513 8.57467 16.7513 9.99967C16.7513 11.4247 15.593 12.583 14.168 12.583H10.8346V14.1663H14.168C16.468 14.1663 18.3346 12.2997 18.3346 9.99967C18.3346 7.69967 16.468 5.83301 14.168 5.83301Z" fill="#0D88BF"></path>
												</svg>
											</template>
						...
						<div class="sidebar">
							<h3><?php esc_html_e( 'What\'s Next', 'jet-booking' ); ?></h3>
								...
								<li>
									...
									>
										<template slot="label">
											...
											<?php esc_html_e( 'Tutorial category', 'jet-booking' ); ?>
										</template>
									</cx-vui-button>
								</li>
								...
								<li>
									...
									>
										<template slot="label">
											...
											<?php esc_html_e( 'Support link', 'jet-booking' ); ?>
										</template>
									</cx-vui-button>
								</li>
								...
								<li>
									...
									>
										<template slot="label">
											...
											<?php esc_html_e( 'YouTube playlist', 'jet-booking' ); ?>
										</template>
									</cx-vui-button>
								</li>
							</ul>
					...
					<div class="jet-content__actions">
						...
						>
							<template slot="label">
								<?php esc_html_e( 'Go to Booking Settings', 'jet-booking' ); ?>
							</template>
						</cx-vui-button>
					</div>
				</template>
				...
				<div v-if="currentStep && currentStep <= lastStep" class="jet-content__actions">
					<cx-vui-button v-if="currentStep" button-style="accent-border" @click="prevStep">
						<template slot="label">
							...
							<?php esc_html_e( 'Back', 'jet-booking' ); ?>
						</template>
					</cx-vui-button>

					<cx-vui-button
						button-style="accent"
						:disabled="1 === currentStep && ! setupData.apartment_post_type.length"
						:loading="loading"
						@click="nextStep"
					>
						<template slot="label">
							<template v-if="currentStep === lastStep">
								<?php esc_html_e( 'Finish setup', 'jet-booking' ); ?>
							</template>

							<template v-else>
								<?php esc_html_e( 'Next step', 'jet-booking' ); ?>
							</template>
			...
			<template v-else>
				<div class="jet-content__heading">
					<h2><?php esc_html_e( 'The booking system is already configured', 'jet-booking' ); ?></h2>
				</div>
				...
				<div class="jet-content__body">
					<div class="cx-vui-component cx-vui-component--equalwidth">
						<div class="cx-vui-component__meta">
							<label class="cx-vui-component__label">
								<?php esc_html_e( 'Reset all the current settings', 'jet-booking' ); ?>
							</label>

							<p class="cx-vui-component__desc">
								<?php esc_html_e( 'If you choose to reset, all existing booking settings will be lost.', 'jet-booking' ); ?>
							</p>
						</div>
						...
						<div class="cx-vui-component__control">
							<cx-vui-button button-style="accent-error" @click="resetDialog = true">
								<template slot="label">
									...
									<?php esc_html_e( 'Reset data', 'jet-booking' ); ?>
								</template>
							</cx-vui-button>
						</div>
					</div>
					...
					<div class="cx-vui-component cx-vui-component--equalwidth">
						<div class="cx-vui-component__meta">
							<label class="cx-vui-component__label">
								<?php esc_html_e( 'Keep data and set up manually', 'jet-booking' ); ?>
							</label>

							<p class="cx-vui-component__desc">
								<?php esc_html_e( 'All the previously configured settings will not be changed, and you can edit them manually.', 'jet-booking' ); ?>
							</p>
						</div>
						...
						<div class="cx-vui-component__control">
							<cx-vui-button
								...
								tag-name="a"
								target="_blank"
								url="<?php  echo esc_url( add_query_arg( [ 'page' => 'jet-abaf-settings' ], admin_url( 'admin.php' ) ) ); ?>"
							>
								<template slot="label">
									<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
									...
									<?php esc_html_e( 'Go to settings', 'jet-booking' ); ?>
								</template>
							</cx-vui-button>
						</div>
					</div>
		...
		>
			<template slot="content">
				<div class="jet-abaf-popup__content">
					...
					<h3><?php esc_html_e( 'Are you sure you want to delete all data?', 'jet-booking' ); ?></h3>
					<div><?php esc_html_e( 'After you confirm to "Delete all data", all settings, booking instances, units, and columns will be removed from the database. You won\'t be able to restore them.', 'jet-booking' ); ?></div>
				</div>
				...
				<div class="jet-abaf-popup__actions">
					...
					<cx-vui-button class="compact" button-style="accent-error" @click="goToReset">
						<template slot="label">
							...
							<?php esc_html_e( 'Delete all data', 'jet-booking' ); ?>
						</template>
					</cx-vui-button>
				</div>
			</template>
