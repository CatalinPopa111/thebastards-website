...
class Manager {
	...
	public function __construct( $pages = array() ) {
		...
		add_action( 'admin_menu', [ $this, 'register_pages' ], 9999 );

		if ( $this->is_dashboard_page() ) {
			add_action( 'admin_enqueue_scripts', array( $this, 'assets' ) );
			$page               = ! empty( $_GET['page'] ) ? sanitize_text_field( wp_unslash( $_GET['page'] ) ) : false; // phpcs:ignore WordPress.Security.NonceVerification
	 ...
	 * Register pages.
	 *
	 * Registers and sets up pages for the application.
	 *
	 * @since  1.0.0
	 * @since  4.0.0 Added unique pages capabilities.
	 * @access public
	 *
	 * @return void
	 */
