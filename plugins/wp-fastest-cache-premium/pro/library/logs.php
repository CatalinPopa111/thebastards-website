<?php
	class WpFastestCacheLogs{
		...
		public function action($from = ""){
			if($this->type == "delete"){
				$log = new stdClass();
				$log->date = current_time('timestamp');
		...
		public function getVia(){
			...
			if($via[4]["function"] == "clear_cache_after_woocommerce_order_status_changed"){
				$arr["args"] = array();
		...
		public function decodeVia($data){
			...
			}else if($data->function == "clear_cache_after_woocommerce_order_status_changed"){
				if(isset($data->args) && is_array($data->args)){
					return "<span>- Order Status Changed</span><br><span>- Product ID: ".implode(",", $data->args)."</span>";
				}
		...
		public function printLogs(){
			...
			<div id="delete-logs" style="display:none; padding-bottom:10px;">
				...
				<h2 style="padding-left:20px;padding-bottom:10px;">Clear Cache Logs</h2>
							...
							<?php if(count($this->logs) > 0){ ?>
								<?php foreach ($this->logs as $key => $log) { ?>
									<tr>
										...
										<?php if(is_numeric($log->date)){ ?>
											<th style="vertical-align:top;border-left:1px solid #DEDBD1;" scope="row"><?php echo date_i18n(get_option('date_format')." @ ".get_option('time_format'), $log->date); ?></th>
										<?php }else{ ?>
