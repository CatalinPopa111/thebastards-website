<?php
	class WpFastestCacheImageOptimisation{
		...
		public function __construct(){
			...
			if(isset($_GET["id_manually"]) && $_GET["id_manually"]){
				$this->id = intval($_GET["id_manually"]);
			}
		...
		public function get_server_list(){
			$servers = '{
					    ...
					    "euw": [
					   	...
					   	"eue": [
							   {"key": "at", "time":0,"flag":"bg","location":"At","color":"red", "url":"https://at.wpfastestcache.com"},
		...
		public function get_total_excluded_image_number(){
			$query_images_args = array();
			...
			$query_images_args['meta_query'] = array(
										...
										array(
							                'key'     => '_wpfc_optimize_exclude',
							                'compare' => 'EXISTS' // not excluded
								            )
										);
		...
		public function get_total_pending_image_number(){
			...
			$query_images_args['meta_query'] = array(
										...
										array(
									        'key'     => '_wpfc_optimize_exclude',
									        'compare' => 'NOT EXISTS' // only include images not marked as excluded
									    )
										);
		...
		public function set_server_location() {
		    if (!empty($_GET["server_url"])) {
		        $url = sanitize_text_field($_GET["server_url"]);

		        // Sunucu URL'si beklenen domain formatında mı?
		...
		public function getFirstId(){
			...
			$query_images_args = array(
			    ...
			    'meta_query' => array(
					...
					array(
		                'key'     => '_wpfc_optimize_exclude',
		                'compare' => 'NOT EXISTS' // not excluded
		            )
				)
			);
		...
		public function wpfc_update_image_list_ajax_request(){
			if(!wp_verify_nonce($_REQUEST["nonce"], 'wpfc-statics-nonce')){
				die( 'Security check' );
			}
			...
			if(current_user_can('manage_options')){
				...
				if(isset($_GET["filter"]) && $_GET["filter"]){
					if($_GET["filter"] == "error_code"){
					...
					}else if($_GET["filter"] == "excluded"){

						$query_images_args['meta_query'] = array();

					    $filter_excluded = array(
					        'key'     => '_wpfc_optimize_exclude',
					        'compare' => 'EXISTS'
					    );
					    ...
					    // Add to meta query
					    array_push($query_images_args['meta_query'], $filter_excluded);
		...
		public function get_credit(){
		    ...
		    $option_key     = "WpFc_credit";
		    $cache_lifetime = 6 * 60 * 60; // 6 saat

		    $data = get_option($option_key);

		    $credit     = isset($data['credit']) ? (int)$data['credit'] : false;
		    $saved_time = isset($data['time']) ? (int)$data['time'] : 0;

		    $is_expired = true;

		    if ($credit !== false && $saved_time > 0) {
		        if ((time() - $saved_time) < $cache_lifetime) {
		            $is_expired = false;
		        }
		    }
		    ...
		    // ❌ CACHE YOKSA VEYA 6 SAATTEN ESKİYSE → CURL
		    $domain = preg_replace("/^www\d*\./", "", $_SERVER["HTTP_HOST"]);

		    $response = wp_remote_get(
		        "https://api.wpfastestcache.net/user/" . $domain . "/xcredit/" . get_option("WpFc_api_key"),
		        [
		            'timeout'   => 10,
		            'sslverify' => false,
		        ]
		    ...
		    // ❌ CURL HATASI → ESKİ KAYIT VARSA ONU DÖN
		    if (!$response || is_wp_error($response)) {
		        return ($credit !== false) ? $credit : "Request failed";
		    }
		    ...
		    // ❌ API 200 dönmediyse → eski varsa onu döndür
		    return ($credit !== false) ? $credit : "Error Occured";
		}
		...
		public function wpfc_statics_ajax_request(){
			...
			if(current_user_can('manage_options')){
				$res = array(
							...
							"excluded" => $this->get_total_excluded_image_number(),
							"reduction" => $this->get_total_reduction_image_number(),
							"percent" => 0,
							"credit" => $this->get_credit()
							);
				$res["optimized"] = $res["total_image_number"] - $res["pending"] - $res["error"] - $res["excluded"];

				if($res["total_image_number"] > 0){
					$res["percent"] = ($res["optimized"])*100/($res["total_image_number"] - $res["excluded"]);
				}else{
					$res["percent"] = 0;
				}
				
				$res["percent"] = number_format($res["percent"], 2);
				$res["reduction"] = $res["reduction"]/1024;


				if ($res["reduction"] >= 1000) {
				    $res["reduction"] = round($res["reduction"] / 1024, 2) . " MB";
				} else {
				    $res["reduction"] = round($res["reduction"], 2) . " KB";
				}
				...
				if($this->id){
					...
					if(count($std) == 1 && isset($std[0]) && $std[0]){
						if(isset($std[0]->error_code) && $std[0]->error_code){
							if($std[0]->error_code == 18 || $std[0]->error_code == 5){
...
public function image_list_content($query_images_args = array()){
    ...
    if(count($query_image->posts) > 0){
        foreach ($query_image->posts as $key => $post) {
            ...
            if($is_excluded){
                // Render excluded image template
                if($content = @file_get_contents($this->get_template_path("image_line_excluded.html"))){

                    $url = get_edit_post_link($post->ID);

                    $attachment = wp_get_attachment_url($post->ID);
                    if(!$attachment){
                        $attachment = plugins_url("wp-fastest-cache")."/images/no-image.gif";
                    }
                    ...
                    $datas = array(
                        ...
                        $post->post_title,
                        $url,
                        get_the_date("d-m-Y <br> H:i:s", $post),
    ...
    return $return_output;
		...
		public function get_country_code($sslverify = false) {
		    $data = get_option("wpfc_country_code");

		    if (is_array($data) && isset($data['country']) && isset($data['time'])) {
		        if (time() - $data['time'] < 60*60*24*3) { // 3 gün
		            return $data['country'];
		        }
		    }
		    ...
		    if (wp_remote_retrieve_response_code($response) == 200) {
		        ...
		        if (strlen($country) == 2) {
		            ...
		            update_option("wpfc_country_code", array(
		                'country' => $country,
		                'time'    => time()
		            ));
