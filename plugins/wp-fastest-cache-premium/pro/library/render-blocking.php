	...
	class WpFastestCacheRenderBlocking{
		...
		public function set_js(){
			...
			foreach ($script_tag as $key => $value) {
				...
				if(preg_match("/^<script[^\>]*>\s*\!function\([fbevnts\,\s]+\)\s*\{(((?!\.insertBefore\().)+)\.insertBefore\([^\)]+\)\}\([^\)]+fbevents\.js[^\)]+\);\s*<\/script>/is", $value["text"])){
				...
				if(preg_match("/fbq\(\s*[\"\'][a-z]+[\"\']/i", $value["text"])){
					// <script>
				...
				*/
				...
				// <script id="infogram_0_16111-dc80-4d8a-be7e-ba8bd7328e" title="qqqqq" src="https://e.infogram.com/js/dist/embed.js?TLS" type="text/javascript"></script>
				if(preg_match("/\/\/e\.infogram\.com\/js\/dist\/embed\.js/i", $value["text"])){
					continue;
				}
				...
				*/
				if(preg_match("/<script[^\>]*>\s*var\s+blogherads/i", $value["text"])){
					continue;
				}
				...
				if($value["text"] && preg_match("/^<script/i", $value["text"])){
					if(preg_match("/gravatar\.com\/js\/gprofiles\.js/i", $value["text"]) || 
	    				...
	    				*/
						...
						if(preg_match("/wp-preferences-js-after/i", $value["text"])){
		...
		public function defer_load_inline_js($script){
			...
			if(preg_match("/id\s*\=\s*[\"\']\s*cmplz-cookiebanner-js-after\s*[\"\']/", $script)){
				// <script id="cmplz-cookiebanner-js-after">
				return $script;
			}

			if(preg_match("/optimization-detective\\\\\/detect\.min\.js/i", $script)){
				// import detect from "https:\/\/site.com\/wp-content\/plugins\/optimization-detective\/detect.min.js
				return $script;
			}
