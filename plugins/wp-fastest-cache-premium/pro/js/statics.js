var WpFcStatics = {
	...
	update_image_list: function(page, search){
		...
		jQuery.ajax({
			type: 'GET',
			url: self.ajax_url,
			data : {"action": "wpfc_update_image_list_ajax_request", "page": page, "search" : search, "filter" : filter, 'per_page' : self.per_page, 'nonce' : jQuery('#wpfc-image-static-panel').data('nonce')},
			dataType : "json",
			cache: false, 
	...
	set_click_event_show_hide_button: function(){
		...
		jQuery("div[data-click-action='excluded']").click(function(){
			jQuery("#show-image-list").hide();
			jQuery("#hide-image-list").show();
			jQuery("#wpfc-image-list").show();
			jQuery("#wpfc-image-static-panel").hide();
			jQuery("#wpfc-image-list-filter").val("excluded");
			self.update_image_list(0);
		});
	},
	wpfc_get_server_time: function(servers){
		...
		jQuery.ajax({
			...
			data : {"action" : "get_server_time_ajax_request", "servers" : servers, "nonce"  : jQuery('#wpfc-image-static-panel').data('nonce')},
			dataType : "json",
			cache: false,
	...
	wpfc_get_servers: function(){
		...
		if(self.country_code){
			if (servers[self.country_code]) {
		        self.wpfc_get_server_time(servers[self.country_code]);
		    } else if(america.search(self.country_code) > -1){
	...
	optimize_image: function(self, id, recursive, last){
		...
		jQuery.ajax({
			...
			url: self.ajax_url,
			dataType : "json",
			data : {"action": "wpfc_optimize_image_ajax_request", "id" : id, "last" : last, "server_url": server_url, 'nonce' : jQuery('#wpfc-image-static-panel').data('nonce')},
			cache: false,
			timeout: 20000,
			error: function(x, t, m) {
	...
	update_statics: function(callback){
		...
		jQuery.ajax({
			...
			url: self.ajax_url,
			dataType : "json",
			data : {"action": "wpfc_statics_ajax_request", "credit" : credit, 'nonce' : jQuery('#wpfc-image-static-panel').data('nonce')},
			cache: false, 
			success: function(data){
	...
	set_click_event_revert_image: function(){
		...
		jQuery("div.revert").click(function(e){
			...
			jQuery.ajax({
				...
				url: self.ajax_url,
				dataType : "json",
				data : {"action": "wpfc_revert_image_ajax_request", "id" : id, 'nonce' : jQuery('#wpfc-image-static-panel').data('nonce')},
				cache: false, 
				success: function(data){
					try{
