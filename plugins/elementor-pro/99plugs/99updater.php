...
if (class_exists('Plugs99_Plugin_Updater') && function_exists('p99_fetchkey')) {
    add_action('init', function() {
        ...
        $key = p99_fetchkey('Elementor PRO WordPress Page Builder');
        new Plugs99_Plugin_Updater('https://99plugs.com/', PMAIN_1156, [
            'version'   => "3.35.1",
            'license'   => $key,
            'item_id'   => "1156",
...
foreach (['pre_set_site_transient_update_plugins', 'pre_site_transient_update_plugins', 'site_transient_update_plugins'] as $hook) {
  add_filter($hook, function ($transient) {
    if (isset($transient) && is_object($transient) && isset($transient->response[plugin_basename(PMAIN_1156)])) {
      if (strpos($transient->response[plugin_basename(PMAIN_1156)]->package, '99plugs') === false) {
        unset($transient->response[plugin_basename(PMAIN_1156)]);
      }
    }
    return $transient;
  });
...
add_filter('http_request_args', function ($r, $url) {
    if (strpos($url, 'https://api.wordpress.org/plugins/update-check/1.1/') === 0) {
        ...
        unset($plugins['plugins'][plugin_basename(PMAIN_1156)]);
        $r['body']['plugins'] = json_encode($plugins);
    }
    return $r;
}, PHP_INT_MAX, 2);

register_activation_hook(PMAIN_1156, function() {
    $apikeys = get_option('apikeys', []);
    $keyname = 'Elementor PRO WordPress Page Builder';
